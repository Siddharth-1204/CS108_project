...
class Othello_class:
    ...
    def board_generate(self):
        # create empty board and place initial 4 coins
        ...
        for i in range(8):
            for j in range(8):
                if self.is_valid_move(turn, board_array, (i, j)):
                    return True
        return False
    def changed_positions(self, current_turn, board_array, current_move):
        # this function finds all positions that will flip for a move
        array_x = np.array([])
        array_y = np.array([])
        horizontal_line = board_array[current_move[0],:]
        vertical_line = board_array[:,current_move[1]]
            
        # split row into left and right parts
        horizontal_1st_array = horizontal_line[ : current_move[1]]
        horizontal_2nd_array = horizontal_line[current_move[1]+1 : ]

        # check left side: find nearest same-color piece and ensure only opponent pieces in between
        ...
        return np.array(array_x, dtype = int), np.array(array_y, dtype = int)
    def is_valid_move(self, current_turn, board_array, current_move):
        if board_array[current_move[0]][current_move[1]] == 0:
            if (self.changed_positions(current_turn, board_array, current_move)[0] != current_move[0]).any():
                return True
            if (self.changed_positions(current_turn, board_array, current_move)[1] != current_move[1]).any():
                return True
        return False
    def next_board_state(self, current_turn, board_array, current_move):
        if board_array[current_move[0]][current_move[1]] == 0:
            array_x, array_y = self.changed_positions(current_turn, board_array, current_move)
            board_array[array_x,array_y] = current_turn
    ...
    def display(self, screen, board_array, font, username1, username2, turn, is_game_ended, winner):
        ...
        if not is_game_ended:
            ...
            text_surface_3 = font.render("Press space to continue", False, (64, 64, 64))
        else :
        ...
        if is_game_ended :
            text_rect_3 = pygame.Surface.get_rect(text_surface_3, midtop = (500, 60))
    ...
    def animate(self, screen, board_array, current_move, current_turn, started_animating):
        global r, dr, arr_x, arr_y, stage, R, dR, new_not_changing_x, new_not_changing_y
        screen.blit(self.board, self.board_rect)
        if started_animating:
            # get positions to animate (flipping coins)
            arr_x, arr_y = self.changed_positions(current_turn, board_array, current_move)
            stage = 1
            r=30
            dr=3
            R=0
            dR=3
            array = np.stack((arr_x, arr_y), axis = 1)
            ...
            arr_x = np.array(arr_x, dtype = int)
            arr_y = np.array(arr_y, dtype = int)
        elif stage == 1:
            # shrink old coins and grow new ones
            r-=dr
            R+=dR
            if r-dr<0:
                dr = r
            ...
            work = display_vector(arr_y, arr_x, r)
            if r==0 and R == 30:
                stage = 2
                dr = 3
        elif stage == 2:
            # expand new coin to full size
            r+=dr
            if r+dr>30:
                dr = 30-r
